...
public:
	...
	static bool input_in(Type& a) {
		a.X.resize(a.X.size() + 1);
		cout << "Введите данные о студенте:\nНомер З.К" << setw(12) << "Фамилия" << setw(18) << "Номер группы" << setw(11.5) << "Оценки >> ";
		cin >> a.X[a.X.size() - 1].id >> a.X[a.X.size() - 1].firstname >> a.X[a.X.size() - 1].name >> a.X[a.X.size() - 1].group_number;
		if (a.X[a.X.size() - 1].group_number.length() > 10) { return false; }
		for (int i = 0; i < 5; i++) {
			cin >> a.X[a.X.size() - 1].evl[i]; if (a.X[a.X.size() - 1].evl[i] < 1 || a.X[a.X.size() - 1].evl[i] > 5) { return false; }
		}
		return true;
	}
